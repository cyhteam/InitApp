...
+ (id)primaryKeyValueForNewInstance
{
    ...
    if (databaseIsOpen && g_tablesUsingAutoIncrementEmulation && [g_tablesUsingAutoIncrementEmulation containsObject:[self getTableName]]) {
...
//- (instancetype)init { return [self init]; }
- (instancetype)initDefault { return [super init] ;}
        ...
        [g_fieldInfo[self.class] enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
            ...
            if (suppliedValue) {
                ...
                if ([key isEqualToString:g_primaryKeyFieldName[self.class]]) {
                    ...
                    do {
                        attempts++;
                        NSAssert1(attempts < 100, @"FCModel subclass %@ is not returning usable, unique values from primaryKeyValueForNewInstance", [self.class getTableName]);
...
- (id)valueOfFieldName:(NSString *)fieldName byResolvingReloadConflictWithDatabaseValue:(id)valueInDatabase
{
    ...
    [[NSException exceptionWithName:@"FCReloadConflict" reason:
        [NSString stringWithFormat:@"%@ ID %@ cannot resolve reload conflict for \"%@\"", [self.class getTableName], self.primaryKey, fieldName]
    userInfo:nil] raise];
    return nil;
}
...
- (FCModelSaveResult)save
{
    ...
    NSAssert1(primaryKey, @"Cannot update %@ without primary key value", [self.class getTableName]);
...
+ (NSString *)expandQuery:(NSString *)query
{
    ...
    return [query stringByReplacingOccurrencesOfString:@"$T" withString:[self getTableName]];
}
...
+ (BOOL)closeDatabase
{
    ...
    [g_instances enumerateKeysAndObjectsUsingBlock:^(Class class, NSMapTable *classInstances, BOOL *stop) {
        for (id primaryKeyValue in classInstances.keyEnumerator.allObjects) {
            ...
            NSLog(@"[FCModel] closeDatabase: %@ ID %@ is still retained by something and is being abandoned by FCModel. This can cause weird bugs. Don't let this happen.", [class getTableName], primaryKeyValue);
        }
    }];
